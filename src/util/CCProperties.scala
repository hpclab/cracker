...
class CCProperties(algorithmName: String, configurationFile : String) extends Serializable
{
	...
	def getImmutable : CCPropertiesImmutable =
	{
		...
		var separatorCC = get("edgelistSeparatorCC", "space")
		if(separator.equals("space")) separator = " "
		if(separatorCC.equals("space")) separatorCC = " "
		...
		val switchLocal = get("switchLocal", "0").toInt
		...
		val outputFileCC = get("outputFileCC", "")
		val coreThreshold = getInt("coreThreshold", 10)
		val invert = get("invert", "false").toBoolean
		
		//############# WITH YARN
		val sparkExecutorInstances = get("sparkExecutorInstances", "-1").toInt
		
		//################## DIAMETER
		val selfFunction = get("selfFunction", "DISTANCE_MAX")
		
		
		val candidateFunction = get("candidateFunction", "MAX")
